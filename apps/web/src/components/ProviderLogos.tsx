// Brand marks from the simple-icons set (CC0) + the current xAI monogram.
// Rendered as quiet monochrome wordmarks in the trust strip.

function Mark({ d, size = 30 }: { d: string; size?: number }) {
  return (
...
const PATHS = {
  ...
  // current xAI monogram (the angular X), shown next to the Grok wordmark
...
const BRANDS: { d: string; label: string }[] = [
  { d: PATHS.anthropic, label: "Anthropic" },
  { d: PATHS.openai, label: "OpenAI" },
  { d: PATHS.gemini, label: "Gemini" },
  { d: PATHS.xai, label: "Grok" },
];

export default function ProviderLogos() {
  return (
    <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-5 text-mute">
      {BRANDS.map((b) => (
        <span key={b.label} className="flex items-center gap-3 transition-colors hover:text-ink">
          <Mark d={b.d} />
          <span className="text-[22px] font-medium tracking-tight">{b.label}</span>
        </span>
      ))}
    </div>
  );
